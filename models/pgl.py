...
from scipy.sparse.linalg import svds # <-- Thêm dòng này
...
class PGL(GeneralRecommender):
    ...
    def global_subgraph_extraction(self, adj):
        ...
        u, s, vt = svds(norm_adj, self.embedding_dim)
        
        # Đảo ngược lại để s giảm dần (giống hành vi của sparsesvd)
        dim = s.shape[0]
        idx = np.argsort(s)[::-1]
        s = s[idx]
        u = u[:, idx]
        ...
        # sparsesvd trả về ut (transpose của u), nên ta cần transpose u
        ut = u.T
    ...
    def print_embed(self):
        """
        Hàm này trả về các embedding riêng biệt của từng modality 
        để trainer.py có thể lưu lại.
        """
        # 1. Lấy feature của Item sau khi qua lớp Linear (trs)
        image_feats = None
        if self.v_feat is not None:
            image_feats = self.image_trs(self.image_embedding.weight)
        ...
        # 2. Lấy embedding của User (PGL chia user thành 2 phần text/image)
        user_text = self.user_text.weight
        user_image = self.user_image.weight

        # Trả về theo đúng thứ tự mà trainer.py đang đợi: 
        # t_final, v_final, t_feat, v_feat
        return user_text, user_image, text_feats, image_feats
