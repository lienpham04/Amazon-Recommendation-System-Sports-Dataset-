...
class Config(object):
    ...
    def _load_dataset_model_config(self, config_dict, mg):
        ...
        # 1. Lấy đường dẫn file hiện tại (.../src/utils/configurator.py)
        current_file_path = os.path.abspath(__file__)
        
        # 2. Lùi ra 2 cấp để về thư mục "src"
        # Cấp 1: .../src/utils
        # Cấp 2: .../src
        src_dir = os.path.dirname(os.path.dirname(current_file_path))
        
        # 3. Trỏ vào thư mục configs (vì nó nằm trong src)
        cur_dir = os.path.join(src_dir, 'configs')
        
        # --- KẾT THÚC SỬA ---

        print(f"\n[DEBUG] Đang tìm config tại: {cur_dir}")
        ...
        for file in file_list:
            if os.path.isfile(file):
                print(f"[DEBUG] ✅ Load thành công: {file}") 
                ...
                print(f"[DEBUG] ❌ Vẫn chưa thấy file: {file}")
    ...
    def _set_default_parameters(self):
        ...
        # valid_metric = self.final_config_dict.get('valid_metric', 'Recall@20').split('@')[0]
        print(f"DEBUG: final_config_dict keys: {self.final_config_dict.keys()}")
        print(f"DEBUG: valid_metric value: {self.final_config_dict.get('valid_metric', 'NOT FOUND')}")
